...
VSPluginMarkerLocalization::Marker2DEKFNode::Marker2DEKFNode(VSD::SimState* mySimState, VSD::SimStateInstance* otherSimStateInstance)
...
VSPluginMarkerLocalization::Marker2DEKFNode::~Marker2DEKFNode()
{
	if (visu_)
		delete visu_;
}

VSDIO::ExtensionIOBoard* VSPluginMarkerLocalization::Marker2DEKFNode::getIOBoard()
{
   if (ioBoard == 0)
   {
...
void VSPluginMarkerLocalization::Marker2DEKFNode::postHasBeenAddedToElementContainer()
{
...
void VSPluginMarkerLocalization::Marker2DEKFNode::preWillBeRemovedFromElementContainer()
{
   VSLibSensorDataProcessing::SensorDataProcessingNodeBase::preWillBeRemovedFromElementContainer();

	timer->stop();
	VSD::Connection::disconnect(timer, SIGNAL(timeout()), this, SLOT(slotSendData()));
}

// note: edit here to process data on input change
void VSPluginMarkerLocalization::Marker2DEKFNode::slotPropertiesModified(VSD::SimStateInstance* simStateInstance, const VSD::MetaProperty* metaProperty)
{
	if (getMyEnvironment()->getOptions() & VSD::Environment::IsSlave)
		return;

	VSPluginMarkerLocalization::Marker2DEKFNode* thisInstance = simStateInstance->instanceCast<VSPluginMarkerLocalization::Marker2DEKFNode*>();
	if (thisInstance)
	{
...
void VSPluginMarkerLocalization::Marker2DEKFNode::slotVelocityModified(VSDIO::Input* input)
{
...
void VSPluginMarkerLocalization::Marker2DEKFNode::slotEnableModified(VSDIO::Input* input)
{
	// get value of the bool input "execute" defined by base class
	if (!getInputValueExecute())
	{
...
void VSPluginMarkerLocalization::Marker2DEKFNode::slotRotationModified(VSDIO::Input* input)
{
	// get value of the bool input "execute" defined by base class
	if (!getInputValueExecute())
		return;
...
void VSPluginMarkerLocalization::Marker2DEKFNode::slotMeasurementModified(VSDIO::Input* input)
{
	// TODO: reset filter when InputValueExecute false !
	// get value of the bool input "execute" defined by base class
	if (!getInputValueExecute())
		return;
...
void VSPluginMarkerLocalization::Marker2DEKFNode::slotUpdatePoseWithOdom()
{
	if (!getInputValueExecute())
		return;
...
// public get fcns
VSM::Vector3 VSPluginMarkerLocalization::Marker2DEKFNode::getRobotPose()
{
	return robotPose_;
}

VSM::Matrix3x3 VSPluginMarkerLocalization::Marker2DEKFNode::getRobotCovariance()
{
	return robotCovariance_;
}

std::unordered_map<int, VSM::Vector3> VSPluginMarkerLocalization::Marker2DEKFNode::getMarkerPoses()
{
	return markerPoses_;
}

std::unordered_map<int, VSM::Matrix3x3> VSPluginMarkerLocalization::Marker2DEKFNode::getMarkerCovariances()
{
	return markerCovariances_;
}

// note: sanity check
bool VSPluginMarkerLocalization::Marker2DEKFNode::propertyCheck()
{
   if(getMyEnvironment()->getOptions() & VSD::Environment::IsSlave)
      return false;
...
bool VSPluginMarkerLocalization::Marker2DEKFNode::loadStaticMapFromFile()
{
...
VSM::Frame VSPluginMarkerLocalization::Marker2DEKFNode::vec2frame(VSM::Vector3 vecIn)
{
	double height = 1;
